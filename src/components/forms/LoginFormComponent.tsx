...
const LoginFormComponent = () => {
  ...
  const handleLoginForm = (evt: { preventDefault: () => void; }) => {
    evt.preventDefault();

    setErrors(errors => ({ ...validateCredentials(credentials) }));
  };

  const validateCredentials = (credentials: { username: any; password: any; }) => {
    let errors = {};
  ...
  const handleInputChange = (evt: { persist: () => void; target: { name: any; value: any; }; }) => {
    evt.persist()
